...
client.on("guildMemberAdd", (member) => {
  ...
  if (!channel) return;
  channel.send(
    `Welcome to the server, ${member}` + ". You are a gamer now:video_game:"
  );
});
...
client.on("message", async (msg) => {
  ...
  if (msg.content === `${prefix}help`) {
    const helpList = new MessageEmbed()
      ...
      .addFields(
        ...
        {
          name: "$clear",
          value: "Clear the specified amount of messages in the chat.",
        }
      );
  ...
  if (args[0] == `${prefix}clear`) {
    ...
    if (deleteCount > 1 && deleteCount <= 100) {
      ...
      msg.channel.bulkDelete(fetched).catch((error) => {
        ...
        if (member) {
          ...
          member
            ...
            .then(() => {
              ...
              msg.reply(
                `Successfully kicked ${user.tag}` + " | " + "Ping time: " + ping
              );
            })
            .catch((err) => {
              // An error happened
        ...
        if (member) {
          ...
          member
            ...
            .then(() => {
              // We let the message author know we were able to kick the person
              msg.reply(
                `Successfully banned ${user.tag}` + " | " + "Ping time: " + ping
              );
            })
            .catch((err) => {
              // An error happened
